...
## Create a customer-scoped order from the Customers workspace (customer_scoped_order_create)
...
- intent_aliases_en: create order from customer page, create order from customer, customer orders tab, customer order workspace, order workspace, create order in customer workspace, create order from customers workspace, customer contract from customer page
- intent_aliases_de: auftrag direkt beim kunden, auftrag im kundenbereich, orders tab beim kunden, orders-tab beim kunden, kundenauftrag im kundenbereich, kundenvertrag im kundenbereich, vertrag im kundenbereich registrieren, auftragsbereich beim kunden
- route_path: /admin/customers/order-workspace
- route_aliases: /admin/customers/new-plan
- entry_points: Customers workspace, Selected customer detail, Orders tab, New order, Edit existing order
- linked_page_ids: C-01, C-02, P-04
- linked_pages_labeled: C-01 Customers Workspace, C-02 Customer Order Workspace, P-04 Staffing Board & Coverage
- api_families: customers, planningOrders, planningShifts, platformDocuments

### Steps

- 1. order-details [page C-02 / module customers]
  ...
  important_fields: order_no, title, service_category_code, requirement_type_id, patrol_route_id, service_from, service_to, security_concept_text, notes, release_state
  ...
  source_basis:
    - frontend_component / CustomerAdminView.vue / C-01 / customers: The customer detail Orders tab starts the order workspace with customer_id from the selected customer context.
    - frontend_component / new-plan-step-content.vue / C-02 / customers: The order-details step loads and saves order_no, title, service category, requirement type, patrol route, service window, security concept, notes, and release_state.
    - frontend_api / planningOrders.ts / C-02 / planning: The order-details step imports listCustomerOrders, getCustomerOrder, createCustomerOrder, and updateCustomerOrder from the planningOrders API family.
- 2. order-scope-documents [page C-02 / module customers]
  ...
  important_fields: equipment_item_id, required_qty, requirement_type_id, function_type_id, qualification_type_id, min_qty, target_qty, notes, document_id
  api_functions: listOrderEquipmentLines, createOrderEquipmentLine, updateOrderEquipmentLine, deleteOrderEquipmentLine, listOrderRequirementLines, createOrderRequirementLine, updateOrderRequirementLine, deleteOrderRequirementLine, listOrderAttachments, createOrderAttachment, linkOrderAttachment, unlinkOrderAttachment, listDocuments
  required_permissions: planning.order.read, planning.order.write
  source_basis:
    - frontend_component / new-plan-step-content.vue / C-02 / customers: The order-scope-documents step contains equipment, requirements, and order-document sections, including duplicate guards and document picker flows.
    - frontend_api / planningOrders.ts / C-02 / planning: The planningOrders API family exposes equipment-line, requirement-line, order-attachment, and document search endpoints used by this step.
- 3. planning-record-overview [page C-02 / module customers]
  ...
  important_fields: name, planning_mode_code, planning_from, planning_to, dispatcher_user_id, notes, status, site_detail_site_id, site_detail_watchbook_scope_note, event_detail_event_venue_id, event_detail_setup_note, trade_fair_detail_trade_fair_id, trade_fair_detail_trade_fair_zone_id, patrol_detail_patrol_route_id, patrol_detail_execution_note
  ...
  source_basis:
    - frontend_component / new-plan-step-content.vue / C-02 / customers: The planning-record-overview step edits planning mode, planning window, dispatcher, notes, status, and planning-mode-specific detail fields while persisting planning_record_id.
    - frontend_api / planningOrders.ts / C-02 / planning: The order workspace imports getPlanningRecord, listPlanningRecords, createPlanningRecord, and updatePlanningRecord for the planning-record step.
- 4. planning-record-documents [page C-02 / module customers]
  ...
  required_permissions: planning.record.read, planning.record.write
  source_basis:
    - frontend_component / new-plan-step-content.vue / C-02 / customers: The planning-record-documents step has its own upload and link flows and is separate from order documents in the wizard layout.
    - frontend_api / planningOrders.ts / C-02 / planning: The planningOrders API family exposes planning-record attachment create, link, list, and unlink endpoints used by this step.
- 5. shift-plan [page C-02 / module customers]
  title_en: Shift plan
  ...
  source_basis:
    - frontend_component / new-plan-step-content.vue / C-02 / customers: The shift-plan step selects or creates a shift plan and persists the shift_plan_id under the wizard state.
    - frontend_api / planningShifts.ts / C-02 / planning: The planningShifts API family exposes listShiftPlans, createShiftPlan, getShiftPlan, and updateShiftPlan for this step.
- 6. series-exceptions [page C-02 / module customers]
  ...
  important_fields: shift_template_id, recurrence_code, interval_count, weekday_mask, timezone, date_from, date_to, local_start_time, local_end_time, default_break_minutes, shift_type_code, meeting_point, location_text, customer_visible_flag, subcontractor_visible_flag, stealth_mode_flag, from_date, to_date, regenerate_existing
  api_functions: listShiftTemplates, createShiftTemplate, getShiftTemplate, listShiftTypeOptions, listShiftSeries, createShiftSeries, getShiftSeries, updateShiftSeries, createShiftSeriesException, listShiftSeriesExceptions, updateShiftSeriesException, deleteShiftSeriesException, generateShiftSeries
  required_permissions: planning.shift.read, planning.shift.write, planning.staffing.read
  source_basis:
    - frontend_component / new-plan-step-content.vue / C-02 / customers: The final wizard step edits shift series, exceptions, generation options, and builds a /admin/planning-staffing handoff route after Generate Series & Continue.
    - frontend_api / planningShifts.ts / C-02 / planning: The planningShifts API family exposes shift template, shift series, shift-series exception, and generateShiftSeries endpoints used by the final step.
