...
test("employee detail uses dashboard and overview top-level tabs with one-page overview sections", () => {
  ...
  assert.match(viewSource, /rootMargin:\s*"-30% 0px -55% 0px"/);
  ...
  assert.match(viewSource, /\.employee-admin-overview-nav\s*\{[\s\S]*position:\s*sticky/);
  assert.match(viewSource, /top:\s*var\(--sp-sticky-offset,\s*6\.5rem\)/);
  assert.match(viewSource, /max-height:\s*calc\(100vh - var\(--sp-sticky-offset,\s*6\.5rem\) - 1rem\)/);
  assert.match(viewSource, /\.employee-admin-overview-nav\s*\{[\s\S]*overflow-y:\s*auto/);
  assert.match(viewSource, /\.employee-admin-overview-section-card\s*\{[\s\S]*scroll-margin-top:\s*var\(--sp-sticky-offset,\s*6\.5rem\)/);
  assert.match(viewSource, /@media \(max-width: 1080px\)[\s\S]*\.employee-admin-overview-nav\s*\{[\s\S]*position:\s*static/);
