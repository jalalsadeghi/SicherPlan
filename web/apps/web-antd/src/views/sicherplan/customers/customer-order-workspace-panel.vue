...
const CUSTOMER_ORDER_WORKSPACE_EMBEDDED_QUERY_MODE_VALUES = new Set(['create', 'edit']);
...
const emit = defineEmits<{
  (event: 'cancel'): void;
  (event: 'completed', payload: { customerId: string; orderId: string }): void;
  (event: 'order-updated', payload: { customerId: string; orderId: string }): void;
  (event: 'saved', payload: { customerId: string; orderId: string }): void;
}>();
...
const customerId = computed(() => props.customerId.trim() || routeCustomerId.value);
const routeOrderId = computed(() => normalizeQueryString(route.query.order_id));
const orderId = computed(() => props.orderId.trim() || routeOrderId.value);
const workspaceMode = computed<'create' | 'edit'>(() => {
  if (!props.embedded) {
    return orderId.value ? 'edit' : props.mode;
  }
  const routeMode = normalizeQueryString(route.query.orderWorkspace);
  if (CUSTOMER_ORDER_WORKSPACE_EMBEDDED_QUERY_MODE_VALUES.has(routeMode)) {
    return routeMode as 'create' | 'edit';
  }
  if (normalizeQueryString(route.query.order_mode) === 'edit' || orderId.value) {
    return 'edit';
  }
  return props.mode;
});
...
function buildWizardRouteQuery() {
  ...
  if (props.embedded) {
    nextQuery.pageKey = props.pageKey || normalizeQueryString(route.query.pageKey) || undefined;
    nextQuery.tab = 'orders';
    nextQuery.orderWorkspace = (wizardState.value.order_id || workspaceMode.value === 'edit') ? 'edit' : 'create';
  } else {
...
async function goToNextStep() {
  ...
  if (canSubmitCurrentStep.value) {
    ...
    try {
      ...
      const nextOrderId = submitResult.savedContext?.order_id ?? wizardState.value.order_id;
      if (submitResult.success && nextOrderId) {
        const payload = {
          customerId: customerId.value,
          orderId: nextOrderId,
        };
      ...
      if (submitResult.success && isFinalStep.value && nextOrderId) {
        emit('completed', {
          customerId: customerId.value,
          orderId: nextOrderId,
        });
...
async function syncWizardRouteState(options?: { force?: boolean; source?: 'external' | 'internal-final' | 'repair' }) {
  ...
  await router.replace({
    path: props.embedded ? route.path : '/admin/customers/order-workspace',
    query: nextQuery,
  });
  if (options?.source === 'internal-final') {
...
<template>
  ...
  <div
    v-else
    class="sp-customer-plan-wizard"
    :class="{ 'sp-customer-plan-wizard--embedded': props.embedded }"
    :data-testid="props.embedded ? 'customer-order-workspace-panel-embedded' : 'customer-order-workspace-panel'"
  >
    <nav
      v-if="!props.embedded"
    ...
    <div v-if="props.embedded" class="sp-customer-plan-wizard__embedded-topbar">
      <button
        type="button"
        class="cta-button cta-secondary"
        data-testid="customer-order-workspace-back-to-orders"
...
.sp-customer-plan-wizard--embedded {
  padding: 0;
}

.sp-customer-plan-wizard__embedded-topbar {
  display: flex;
