...
## SicherPlan Runtime Note

This workspace is intentionally using the official `vue-vben-admin` monorepo and inherits its runtime requirements.

- Node.js: `^20.19.0 || ^22.18.0 || ^24.0.0`
- pnpm: `>=10`
- Recommended local version for this repo: `22.22.0` via [.nvmrc](/home/jey/Projects/SicherPlan/web/.nvmrc)

If `npm run dev` fails with a syntax error around `cli-spinners` or JSON import attributes, your shell is still using Node 18. Switch to Node 22 first:

```bash
cd web
nvm use
pnpm install
pnpm dev --filter=@vben/web-antd -- --host 0.0.0.0 --port 5173
```
